...
module.exports = function (router, shared) {
  ...
  router.get('/api/words/pca', async (req, res, next) => {
    try {
      let words = await Word.findAll()
      words = words.map(x => x.word)
      words = Array.from(new Set(words))
      words = words.map(word => shared.getVector(word))
      let labels = words.map(x => x.word)
      // console.log(words.length)
      // res.json(words)
      let vectors = words.map(x => x.values)
      // res.json(words)
      let reducedVectors = await PCA(vectors)
      let outputData = reducedVectors.map((vector, i) => ({ x: vector[0], y: vector[1], label: labels[i] }))
      res.json(outputData)
    }
    catch (error) {
      next(error)
